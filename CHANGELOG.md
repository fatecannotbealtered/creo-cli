...
- **The creation API is reachable from our own code.** A request arriving on the loopback endpoint now runs on Creo's own thread and reads a session that casts to `wfcSession.WSession` -- the interface that builds element trees, which is how features are created. `/session` reports `"creation_api": true` on this machine against Creo 13 build 2026163, in about 100ms end to end. Everything between an agent issuing a command and Creo making geometry is now a matter of writing the commands.
- The toolkit answers only on the thread Creo drives, and that is structural, not an oversight: Creo talks to this JVM over a single socket whose framing carries no request identifiers, and the Java side keeps one static connection for the whole process, so two threads calling at once interleave halves of two conversations. A call from an HTTP thread simply never returns. Work is therefore queued in plain Java and drained by a toolkit timer created during `start()` -- `wfcSession.CreateTimer`, ticking ten times a second, fired by Creo on the thread it owns. The queue is bounded per tick so a burst cannot hold the CAD system, and a request that arrives while a dialog is open waits instead of failing, because the queue is simply not drained until Creo comes back.
- `wfcSession.InvokeLater` looks like the answer -- PTC documents it as callable from a non-toolkit thread, for exactly this situation -- and it is not one. Its bytecode sends on that same shared connection and then runs the dispatch loop on the calling thread, which is the deadlock it appears to avoid. Tried, hung, read, and replaced by the timer.
- `scripts/reload_otk_service.py` does the whole edit-test loop in one command: stop Creo, compile, sign with `protk_unlock.bat`, write `protk.dat`, start Creo with `PRO_JAVA_COMMAND` in the launching process's own environment, then wait for the service and report what it says. Every one of those steps had to be right at the same time for the application to load, and three of them fail silently, so doing them by hand was the slowest part of getting here.
- Creo runs every Object TOOLKIT Java application in one JVM that it starts once and keeps for the session. A class it has already loaded stays loaded and the jar stays open, which is why rebuilding while Creo is up fails with a file-in-use error and why `--restart` exists. The build now says so when that is the error it hit, and decodes tool output in the console's own code page rather than assuming UTF-8, so the message that explains a failure on a Chinese or Japanese Windows is readable rather than replacement characters.
- The asynchronous route was measured and rejected for creation, not assumed away. An outside process can connect to a running Creo (`pfcasyncmt.dll` has to be loaded by name first -- nothing in `pfcasync.jar` does it), start a synchronous application with `StartJLinkApplication`, and call into it; the round trip works and lands on Creo's thread. But an application started that way gets a plain `pfcSession.Session`, not a `WSession`: it is a J-Link application, and J-Link cannot create features. Only the registry route -- `startup otk_java`, `toolkit object`, which Creo's own trail file calls `synchronous J-OTK` -- yields the creation API. Two further facts from the same experiment: the documented argument order is name, class, start, stop, classpath, text path, display, and `ExecuteTask` reaches only tasks registered by the application whose handle you hold, even though task names collide across applications.
