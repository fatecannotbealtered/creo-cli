...
## Derived interface index

`contract/creoson-interface.json` is generated by `scripts/gen_creoson_interface.py`
from the `jsonSpecs` shipped inside the CREOSON 3.0.2 release archive: 175 published
functions and 23 nested return types, each with a Git blob identity. The specs ship
with CRLF endings while the repository stores them with LF, so identities are computed
over the LF-normalized bytes; that reproduces the `file-assemble.json` hash recorded
below, which `tests/test_creoson_interface.py` asserts rather than assumes.

The index is an interface reference used to check this catalog, not vendored CREOSON
source and not a bundled copy of the server. It exists because the alternative --
hand-transcribing field names for roughly a hundred operations with no live Creo to
catch the mistakes -- is exactly the failure this project should not ship.

`tests/test_creoson_interface.py` enforces four things against it: every operation
targets a published function, forwards only published request fields, supplies every
required one, and promises only response keys the upstream returns, typed as the
upstream types them. Deliberate deviations are enumerated in that file with reasons.
A fifth check closes the other direction: every published function must be either
exposed or named in `NOT_EXPOSED` with a justification, so the uncovered surface is a
decision rather than a backlog. `creo_cli/creoson_spec_ids.py` is generated alongside
the index and carries the per-function blob identities and published response types
that the runtime cites in `reference`.
